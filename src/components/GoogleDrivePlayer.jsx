...
const GoogleDrivePlayer = React.memo(
  ({ videoUrl, title, playbackState, onSkip, isHost, requestedBy }) => {
        ...
        if (!isHost) {
          syncIntervalRef.current = setInterval(() => {
            ...
            if (Math.abs(currentElapsed - playerTime) > 3) {
              console.log(
                `Resyncing: host at ${currentElapsed.toFixed(
                  1
                )}s, local at ${playerTime.toFixed(1)}s`
              );
    ...
    // If participant tries to seek around, snap back to global time
    const handleSeeked = useCallback(() => {
      ...
      const elapsedSeconds = (Date.now() - playbackState.startTime) / 1000;
      if (Math.abs(elapsedSeconds - video.currentTime) > 0.75) {
        video.currentTime = elapsedSeconds;
        video.play().catch(() => {});
      }
    }, [isHost, playbackState]);
